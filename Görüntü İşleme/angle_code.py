    ...
    for keypoint in keypoints.xy:
        if(len(keypoint)>2):
            ...
            if aci < 0:
                direction = 'Left'
                aci = -aci
                output_text = f"Angle: {aci} Direction: {direction} "
            ...
            # Görüntünün sol üst köşesine yazı yazdırma
            cv2.putText(frame, output_text , (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
